...
-- COMPREHENSIVE FIX V3: Dynamic Links and Improved Invites
...
drop policy if exists "Owners can view pending invites" on public.pending_invitations;
create policy "Owners can view pending invites" on public.pending_invitations
  for select using (
    exists (select 1 from public.projects where id = project_id and user_id = auth.uid())
  );

drop policy if exists "Owners can insert pending invites" on public.pending_invitations;
create policy "Owners can insert pending invites" on public.pending_invitations
  for insert with check (
    exists (select 1 from public.projects where id = project_id and user_id = auth.uid())
  );

drop policy if exists "Owners can delete pending invites" on public.pending_invitations;
...
-- NOW ACCEPTS p_base_url for dynamic link generation
...
begin
  ...
  ELSE
    ...
    if exists (select 1 from public.pending_invitations where project_id = p_project_id and email = lower(p_email)) then
        -- Update the existing invite's token/timestamp if needed, or just return existing
